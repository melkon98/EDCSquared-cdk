...
import { UseUpdateUserProfile, useLinkFacebookAccount, useLinkTiktokAccount, useLinkYoutubeAccount } from 'hooks';
...
const LinkYoutubeAccount: React.FC<ProfileProps> = ({
  ...
  const {
    ...
    loading,
  } = useLinkYoutubeAccount();
  const {
    updateProfile,
  } = UseUpdateUserProfile();
  const [advData, setAdvData] = useState<YoutubeAccountAccess | null>(null);
  ...
  useEffect(() => {
    if (facebookAccountData) {
      setProfileState((prev) => {
        ...
        return { ...prev, data: { ...prev.data, youtubeAccountAccess: JSON.parse(facebookAccountData) } }
      })
    }
    const addData: AdvertiserData[] = []
    if (facebookAccountData || data?.youtubeAccountAccess) {
      const facebookData: YoutubeAccountAccess = facebookAccountData !== undefined ? JSON.parse(facebookAccountData) : '' || data?.youtubeAccountAccess || []
        ...
        {advData ? (
          ...
          <>
            <h6 className="head-text font-[700] md:text-[24px] text-[18px] text-[#0E0D0D] w-full text-center">
              There was an error linking your Youtube account.
...
export default withProfile(LinkYoutubeAccount);
